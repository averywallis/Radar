...
class signal(Sprite):
    ...
    def __init__(self, position):
        ...
        self.fxcenter= .5
        self.fycenter = 0.5
        self.speed = math.sqrt(((cpx-100)^2)+((450-cpy)^2))
    def step(self):
        if self.a != 2:
            self.t += 1
        if self.a == 0:
            ...
            #self.x += 3*speed
            #self.y -= 1.5*speed
            self.x += ((cpx-100)/100)*(self.speed/100-self.speed)
            self.y -= ((450-cpy)/100)*(self.speed/100-self.speed)
            if self.x >= cpx:
                self.a = 1
                if self.i >= 0:
                    self.i *= 2
        ...
        if self.a == 1:
            ...
            if self.i >= 0:
                ...
                self.setImage(0)
            self.x -= ((cpx-100)/100)*(self.speed/100)
            self.y += ((450-cpy)/100)*(self.speed/100)
