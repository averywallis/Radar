...
class fop(Sprite):
    ...
    def step(self):
        ...
        if time.time() >= self.t + fopt1 and self.p == 0 and self.a == 1:
                self.p = 2
                self.a = 2
        if time.time() >= self.t + fopt2 and self.p == 0 and self.a == 3:
                self.p = 4
                self.a = 4
...
class triathlon(App):
    ...
    def __init__(self, width, height):
        ...
        finishtxt = TextAsset(text="Finish", width=200, align='center',style='10px Arial', fill=Color(0xff2222,1))
        ...
        Sprite(finishtxt,((((SCREEN_WINDOWX - startpos)/td)*sbr+startpos,500)))
        mpro((0,0))
        fpro((0,50))
        mpre((0,100))
        fpre((0,150))
